...
the basic compile-test-deploy workflow — the "hello world" of Midnight. Based on
the official [midnightntwrk/example-counter](https://github.com/midnightntwrk/example-counter)
(archived 2026-08-14). `npx create-mn-app` no longer generates it: its built-in `hello-world`
template stores a message (`storeMessage`). The contract and simulator code here are current. For
deploying to a network, take the wallet code from `example-bboard`, not from the archived counter
(gotcha #80).
